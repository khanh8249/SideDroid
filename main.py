...
def update_extension_bundle_ids(app_bundle, old_parent_id, new_parent_id):
    ...
    prefix Bundle ID mới của app cha. Bắt buộc phải gọi mỗi khi
    app cha bị đổi Bundle ID (ví dụ do trùng App ID), nếu không
    sẽ dính lỗi IXError "Mismatched bundle IDs" lúc install.
    """
    ...
    for entry in os.listdir(plugins_dir):
        ...
        if old_ext_id.startswith(old_parent_id):
            ...
            # fallback nếu prefix không khớp như mong đợi
            suffix = "." + old_ext_id.rsplit(".", 1)[-1]
...
def setup_extension_provisioning(dev_api, app_bundle, app_name):
    ...
    Với mỗi .appex trong PlugIns/:
      - Tạo App ID riêng (nếu chưa có) đúng theo Bundle ID hiện tại
        của extension (đã được update_extension_bundle_ids xử lý
        trước đó nếu app cha bị đổi tên).
      - Tải Provisioning Profile riêng và ghi vào
        PlugIns/<ext>.appex/embedded.mobileprovision.
    Trả về list đường dẫn các provisioning profile của extension,
    để truyền vào zsign qua nhiều cờ -m.
    """
    ...
    for entry in sorted(os.listdir(plugins_dir)):
        ...
        if not ext_app_id_obj:
            ...
            if not ext_app_id_obj:
                log_error(
                    f"Không tạo được App ID cho extension {entry}: "
                    f"{dev_api.last_error}"
                )
                return None
...
def do_sideload(ipa_path, apple_id, password):
    ...
    # Không chỉ kiểm tra file .pem có tồn tại cục bộ, mà còn phải
    # xác nhận Apple thật sự còn giữ ÍT NHẤT 1 certificate cho team
    # này. Nếu không, file .pem cũ (đã bị revoke/hết hạn phía Apple)
    # sẽ gây lỗi "no current certificates" khi tải provisioning profile.
    ...
    if need_new_cert:
        if local_pem_exists and not existing_certs:
            log_warn(
                "Certificate cục bộ không còn tồn tại trên Apple "
                "(có thể đã bị revoke/hết hạn). Tạo certificate mới."
            )
        ...
        if not app_id_obj:
            ...
            if error_kind == "unavailable":
                ...
                if not app_id_obj:
                    ...
                    if app_id_obj:
                        # QUAN TRỌNG: đổi bundle ID của mọi extension
                        # (PlugIns/*.appex) TRƯỚC khi đổi app cha, để
                        # tránh lỗi IXError "Mismatched bundle IDs".

...
def create_p12_from_cert(cert_pem_path, key_pem_path, output_path, password=""):
    """Tạo file .p12 từ certificate và private key"""
    try:
        cmd = [
        ...
        if result.returncode == 0:
            print(f"[p12] ✅ Đã tạo P12: {output_path}")
            return True
        else:
            print(f"[p12] ❌ Lỗi tạo P12: {result.stderr}")
            return False
        ...
        try:
            ...
            if cert_id:
                ...
                if cert_content:
                    ...
                    print(f"[DeveloperAPI] Đã lưu cert: {cert_pem_path}")
                    
                    # Lưu key PEM
                    with open(key_pem_path, "w") as f:
                        f.write(key_pem)
                    print(f"[DeveloperAPI] Đã lưu key: {key_pem_path}")
                    
                    # Tạo P12
                    create_p12_from_cert(cert_pem_path, key_pem_path, p12_path, password="")
                    print(f"[DeveloperAPI] Đã tạo P12: {p12_path}")
